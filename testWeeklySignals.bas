Attribute VB_Name = "testWeeklySignals"
Option Explicit

' TEST HARNESS - mirrors WeeklySignals.bas. Not imported into production TRADE.xlsm.
' Rename kept subs with Test_ prefix to avoid Ambiguous Name Detected compile errors.
Private Sub GenerateWeeklyTradingSignals_UltraFast()  ' Renamed: canonical in WeeklySignals.bas
    ...
    msgbox "ULTRA FAST: " & signalCount & " signals in " & Format(Timer - startTime, "0.000") & " seconds"
End Sub

Private Function GenerateWeeklySignal_Array(allData As Variant, rowNum As Long) As String
...
Private Function CalculateSignalScore_Fast(rsi As Double, macd As Double, macdSignal As Double, _
...
Private Sub PopulateSignalArray_Array(signals() As Variant, signalCount As Long, allData As Variant, rowNum As Long, signal As String)
...
Private Function CalculateSignalConfidence_Array(allData As Variant, rowNum As Long, signal As String) As Integer
...
Private Function HasCompleteWeeklyData_Array(allData As Variant, rowNum As Long) As Boolean
...
Private Function origGenerateWeeklySignal_Array(allData As Variant, rowNum As Long) As String
...
Private Sub origPopulateSignalArray_Array(signals() As Variant, signalCount As Long, allData As Variant, rowNum As Long, signal As String)
    ' Populate output array using array data only
    Dim entryPrice As Double, atr As Double
    entryPrice = allData(rowNum, 5)
    atr = allData(rowNum, 14)
...
Private Function origCalculateSignalConfidence_Array(allData As Variant, rowNum As Long, signal As String) As Integer
...
Private Sub GenerateWeeklyTradingSignals_Optimized()
...
Private Sub CreateAndOutputSignalsSheet(signals() As Variant, signalCount As Long)
...
Private Sub FastFormatWeeklySignals(ws As Worksheet, signalCount As Long)
...
Private Sub ApplyFastConditionalFormatting(ws As Worksheet, signalCount As Long)
    If signalCount < 1 Then Exit Sub
...
Private Function HasCompleteWeeklyData(ws As Worksheet, rowNum As Long) As Boolean
    On Error GoTo ErrorHandler
...
Private Function GenerateWeeklySignal_Fast(ws As Worksheet, currentRow As Long) As String
...
Private Function origCalculateSignalScore_Fast(rsi As Double, macd As Double, macdSignal As Double, _
                            priceVsMA As Double, compositeScore As Double, _
                            volumeSpike As Double, atrPercent As Double, ibs As Double) As Integer
    Dim score As Integer
    score = 0
    ...
    CalculateSignalScore_Fast = score
End Function
Private Function ulwkNz(value As Variant, Optional defaultVal As Variant = 0) As Variant
...
Private Function GetSignalStrength(signal As String) As String
    Select Case signal
